...
<div class="card" style="width: 100%; padding: 10px;">
  ...
  <form id="monitoringForm" action="{{ route('pts.monitoring-pembayaran.cari') }}" method="POST">
    @csrf

    <div class="row mb-3 mx-2">
      <label class="col-sm-2 col-form-label"><b style="font-size: 12px;">NIDN / NUPTK</b></label>
      <div class="col-sm-10">
        <input type="text" class="form-control" name="nidn" value="{{ old('nidn', $identifier ?? '') }}" placeholder="Masukkan NIDN atau NUPTK">
        @error('nidn')
      ...
      <div class="col-sm-2">
        <select name="start_year" class="form-select" id="start_year">
          @if(!empty($years))
            ...
            @foreach($years as $y)
              <option value="{{ $y }}" {{ (string)$selStart === (string)$y ? 'selected' : '' }}>{{ $y }}</option>
            @endforeach
          @else
            <option value="">-</option>
          @endif
      ...
      <div class="col-sm-2">
        <select name="end_year" class="form-select" id="end_year">
          @if(!empty($years))
            ...
            @foreach($years as $y)
              <option value="{{ $y }}" {{ (string)$selEnd === (string)$y ? 'selected' : '' }}>{{ $y }}</option>
            @endforeach
          @else
            <option value="">-</option>
          @endif
    ...
    @if ($transaksi)
      <div class="row mb-3 mx-2">
        ...
        <div class="col-sm-10">
          <input type="text" class="form-control" readonly style="background-color: #eceef1;"
            value="{{ ($transaksi->NIDN ?? $identifier) }} - {{ $transaksi->Nama ?? '-' }}">
        </div>
      </div>
      ...
      <div class="row mb-3 mx-2">
        ...
        <div class="col-sm-10">
          <input type="text" class="form-control" readonly style="background-color: #eceef1;"
            value="{{ ($transaksi->JabatanSelected ?? $transaksi->Jabatan12 ?? '-') }} - {{ ($transaksi->Aktif ?? 0) == 1 ? 'Aktif' : 'Tidak Aktif' }}">
        </div>
      </div>
        ...
        <div class="col-sm-6">
          <div class="d-flex gap-2 align-items-end">
            <div>
              ...
              <select id="filterTahunVersi" name="tahun_versi" class="form-select" data-ajax-table-url="{{ route('pts.monitoring-pembayaran.table') }}">
                @foreach(($yearsForNidn ?? []) as $y)
                  <option value="{{ $y }}" {{ (string)($selectedYear ?? null) === (string)$y ? 'selected' : '' }}>{{ $y }}</option>
                @endforeach
              </select>
            </div>
  ...
  @if ($transaksi)
    <form id="exportForm" action="{{ route('pts.monitoring-pembayaran.export-excel') }}" method="POST" class="d-none">
      @csrf
      <input id="exportTahunVersi" type="hidden" name="tahun_versi" value="{{ $selectedYear ?? '' }}">
    </form>

    <form id="cetakSptForm" action="{{ route('pts.monitoring-pembayaran.cetak-spt') }}" method="POST" class="d-none">
      @csrf
      <input id="cetakSptTahunVersi" type="hidden" name="tahun_versi" value="{{ $selectedYear ?? '' }}">
      <input type="hidden" name="nidn" value="{{ $identifier ?? '' }}">
    </form>

    <div id="monitoringTableContainer">
      @include('pts.partials.monitoring-pembayaran-table')
    </div>
  @endif
</div>
@endsection
